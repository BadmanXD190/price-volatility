...
@app.get("/api/predictions/path")
def path(item_code: int = Query(...), state: str = Query(...)):
    df = get_df()
    out = df[(df.item_code == item_code) & (df.state == state)].sort_values("target_date")
